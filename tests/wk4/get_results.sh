...
for dir in submissions/*; do
    ...
    if [ -z "$(ls -A $dir | grep test_results.txt)" ]; then
        ...
        test_demo1=$(cat $dir/test_results.txt | grep "test_demo_skbin1 PASSED" | wc -l)
        test_demo2=$(cat $dir/test_results.txt | grep "test_demo_skbin2 PASSED" | wc -l)
        ...
        echo "Demo skbin1.py... [ $test_demo1 ]"
        echo "Demo skbin2.py... [ $test_demo2 ]"
    fi
done